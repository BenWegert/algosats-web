  ...
  return call.then(function (res) {
    ...
    return res.text().then(function (text) {
      ...
      try {
        ...
        error = new Error(
          json.msg || "".concat(res.status, " ").concat(res.statusText)
        );
        ...
        error = new Error(
          "".concat(res.status, " ").concat(res.statusText, " ").concat(text)
        );
        error.response = res;
...
var checkParams = function checkParams(name, payload) {
  ...
  requires.forEach(function (r) {
    if (!payload[r] && isNaN(payload[r])) {
      throw new Error(
        "Method ".concat(name, " requires ").concat(r, " parameter.")
      );
    }
  });
  return true;
...
var publicCall = function publicCall(_ref) {
  ...
  return function (path, data) {
    ...
    return sendResult(
      fetch(
        ""
          .concat(!path.includes("/fapi") ? endpoints.base : endpoints.futures)
          .concat(path)
          .concat(makeQueryString(data)),
        {
          method: method,
          json: true,
          headers: headers,
...
var privateCall = function privateCall(_ref3) {
  ...
  return function (path) {
    ...
    return (data && data.useServerTime
      ? pubCall("/api/v1/time").then(function (r) {
          return r.serverTime;
        })
      ...
      return sendResult(
        fetch(
          ""
            .concat(
              !path.includes("/fapi") ? endpoints.base : endpoints.futures
            )
            .concat(path)
            .concat(noData ? "" : makeQueryString(newData)),
          {
            method: method,
            headers: {
              "X-MBX-APIKEY": apiKey,
...
var _book = function book(pubCall, payload) {
  ...
  return (
    ...
    pubCall(endpoint, payload).then(function (_ref4) {
...
var _aggTrades = function aggTrades(pubCall, payload) {
  ...
  return (
    ...
    pubCall(endpoint, payload).then(function (trades) {
...
var _default = function _default(opts) {
  ...
  var pubCall = publicCall(
    _objectSpread({}, opts, {
      endpoints: endpoints,
    })
  );
  var privCall = privateCall(
    _objectSpread({}, opts, {
      endpoints: endpoints,
      pubCall: pubCall,
    })
